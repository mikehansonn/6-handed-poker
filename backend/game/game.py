...
class TexasHoldem:
    ...
    def get_create_game_json(self) -> dict:
        """
        Returns a simplified JSON representation of the initial game state,
        including global game values and a simplified list of players.
        """
        # Build a simplified list of players
        players_list = []
        for i, player in enumerate(self.players):
            players_list.append({
                "name": player.name,
                "position": self.get_player_position(i),
                "chips": player.chips,
                "status": player.is_active.value  # Assuming Status enum's value is a string like "active"
            })
