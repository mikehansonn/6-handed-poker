...
const ActionButtons = ({ gameState, handleActionClick, betAmount, setBetAmount, handlePlayerAction }) => {
  ...
  const buttonStyles = {
    ...
    call: "bg-green-500 hover:bg-green-600 border-b-4 border-green-700",
  ...
  return (
    isUserTurn && (
      <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50 flex flex-row items-center justify-center">
        {/* Horizontal container for all controls */}
        <div className="flex flex-row items-center gap-4 bg-gray-800/80 p-5 rounded-lg backdrop-blur-sm">
              ...
              .map(action => (
                <div key={action} className="relative group">
                  <button
                    ...
                    className={`${buttonStyles[action]} text-white px-6 py-3 rounded-lg font-semibold transform hover:scale-105 transition-all duration-200 shadow-lg uppercase tracking-wide relative`}
                  >
                    <span>{action}</span>
